...
async function getMonitors() {
  try {
    ...
    monitorsDropdown.innerHTML = '<option value="-1">Entire Screens</option>';
...
async function getWindowSources() {
  try {
    ...
    if (windowSources.length === 0) {
      sourceThumbnails.innerHTML =
        '<div class="no-sources">No window sources found</div>';
...
// Set up video stream for the entire screens
async function setupStreamForEntireScreen() {
  try {
    if (stream) {
      stream.getTracks().forEach((track) => track.stop());
    }
...
async function setupStreamForMonitor(monitorId) {
  try {
    ...
    if (screenSources.length > 0) {
      const sourceId = screenSources.find(
        (source) => monitorId == source.display_id
      ).id;
...
monitorsDropdown.addEventListener("change", () => {
  ...
  if (monitor) {
    ...
    selectedMonitor = null;
    startBtn.disabled = false;
    setupStreamForEntireScreen();
    setStatus("Recording entire screens");
  } else {
    selectedMonitor = null;
    startBtn.disabled = true;
